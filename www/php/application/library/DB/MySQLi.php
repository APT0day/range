...
class MySQLi {
    ...
    private function connect() {
        ...
        if($this->db_link->connect_errno) {
            throw new MySQLiException('Connect Error: ' . $this->db_link->connect_errno);
        }
        if(!empty($this->config['charset'])) {
            $this->db_link->set_charset($this->config['charset']);
            // 让数据库里的 int 和 float 返回成对应的 PHP 数字类型
            $this->db_link->options(MYSQLI_OPT_INT_AND_FLOAT_NATIVE, true);
        }
        return true;
    }
        ...
        for ($i = 0; $i < 2; $i++) {
            ...
            if ($result === false) {
                if ($this->db_link->errno == 2013 or $this->db_link->errno == 2006 or ($this->db_link->errno == 0 and !$this->db_link->ping())) {
                    ...
                    throw new MySQLiException(__CLASS__ . " SQL Error".$this->errorMessage($params[0]));
                }
            }
            break;
        }
    ...
    public function query($sql, $resultmode = null) {
        ...
        if (!$result) { //false
            throw new MySQLiException(__CLASS__ . " SQL Error:" . $this->errorMessage($sql));
        }
    ...
    public function exec($sql){
        ...
        if($result===true)
            ...
            throw new MySQLiException('your sql cannot use exec,use query instead');
    }
    ...
    public function insert($data){
        ...
        foreach ($data as $k=>$v){
            if(is_int($k))
                throw new MySQLiException('invalid data');
            $keys[] = '`' . $k . '`';
            if(is_string($v)){
    ...
    private function packWhereStatement($where){
        ...
        //]]
        if(empty($where))
            throw new MySQLiException('where statement cannot be empty');
        $tpl = [];
    ...
    private function checkNecessary(){
        if(!$this->table)
            throw new MySQLiException('table not set');
    }
}
